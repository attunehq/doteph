...
| `eph system prune [--dry-run] [--include-legacy]` | Global stale-workspace prune. Removes resources for recorded workspace paths that are missing or empty. Skips legacy state unless `--include-legacy` is passed. Verifies `run=` process identity before signaling a PID; warns when it cannot prove the PID is still eph's process. |
