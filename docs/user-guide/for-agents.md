...
| `eph system prune [--dry-run] [--compatibility-v042]` | Global stale-workspace prune. Removes resources for recorded workspace paths that are missing or empty. Skips v0.4.2-and-earlier state unless `--compatibility-v042` is passed. Verifies `run=` process identity before signaling a PID; warns when it cannot prove the PID is still eph's process. |
