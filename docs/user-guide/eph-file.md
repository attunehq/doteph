...
- **At runtime**, `eph env`, hooks, `eph run`, and every service's own
  ...
  `env.<KEY>=` is resolved into the process it launches. A reference that
  parsed fine but names a service that is not running right now is left in
  place verbatim rather than blanked, so an unresolved reference stays visible
