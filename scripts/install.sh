...
# eph installer script
#
# Usage:
#   curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash
#   curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash -s -- -b /usr/local/bin
#   curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash -s -- -v 0.1.0
#
# Options:
...
detect_platform() {
  ...
  case "$kernel" in
    ...
    Darwin)
      ...
      ;;
    MINGW* | MSYS* | CYGWIN*)
      fail "Windows is not supported by this installer. Use scripts/install.ps1 from PowerShell."
      ;;
    *)
  ...
  # Linux ships both glibc and fully static musl builds; pick the right one.
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -h|--help)
        echo "eph installer"
        echo
        echo "Usage: curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash [args]"
        echo
        echo "Options:"
        ...
        echo "  curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash"
        echo
        echo "  # Install specific version"
        echo "  curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash -s -- -v 0.1.0"
        echo
        echo "  # Install to custom directory"
        echo "  curl -sSfL https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.sh | bash -s -- -b /usr/local/bin"
        exit 0
        ;;
      *)
...
install_binary() {
  ...
  local bin_dir="$3"
  local tmp_dir="$4"
  local archive_name="eph-${platform}.tar.gz"
  local binary_name="eph"
  ...
  cp "$extracted_binary" "$bin_dir/eph"
  chmod +x "$bin_dir/eph"
  ...
  installed_version=$("$bin_dir/eph" --version 2>/dev/null || echo "eph")
  info "Installed '$installed_version' to '$bin_dir/eph'"
...
main() {
  ...
  echo "" >&2
  info "Installation complete! Run 'eph --help' to get started."
}
