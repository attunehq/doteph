#!/usr/bin/env pwsh
# eph installer script for Windows (PowerShell)
#
# Usage:
#   irm https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.ps1 | iex
#   $env:Version="0.1.0"; irm https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.ps1 | iex
#   $env:BinDir="C:\Tools"; irm https://raw.githubusercontent.com/attunehq/doteph/main/scripts/install.ps1 | iex
#
...
function Install-Binary {
    ...
    $tempExtractDir = Join-Path $TempDir "eph-install-$(Get-Random)"
    ...
    $extractedDir = Join-Path $tempExtractDir "eph-$Platform"
    $binaryPath = Join-Path $extractedDir "eph.exe"
    $targetPath = Join-Path $InstallDir "eph.exe"
    ...
    Write-Host ""
    Write-Host "Run 'eph --help' to get started"
}
...
function Main {
    ...
    if ([string]::IsNullOrEmpty($BinDir)) {
        $BinDir = Join-Path $env:LOCALAPPDATA "Programs\eph"
    }
